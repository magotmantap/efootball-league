...
/* ============================================================
   EFOOTBALL ELITE LEAGUE — one file, no passwords, no API.

   ┌─ TO SHARE ONE LEAGUE WITH EVERYONE ─────────────────────┐
   │ Paste your Upstash details on the two lines below.      │
   │ Leave them empty and it saves in your browser only.     │
   └─────────────────────────────────────────────────────────┘

   Get them at console.upstash.com → your database → REST API.
   ============================================================ */

const DB_URL = "";    // e.g. "https://frank-mutt-12345.upstash.io"
const DB_TOKEN = "";  // the long token next to it
...
const HOME_FACTOR = 1.10;    // home advantage on expected goals
const K_LEAGUE = 5;          // how much a league match moves ratings
const FRIENDLY_WEIGHT = 0.35;// friendlies count 35% of a league match
const K_FRIENDLY = K_LEAGUE * FRIENDLY_WEIGHT;
const START_COINS = 1000;
const MIN_STAKE = 10;
const MAX_ODDS = 25;
const SIMS = 1200;           // simulated seasons for title odds
...
const STORE_KEY = "efl:league:v3";
const ME_KEY = "efl:me:v3";
const LOCAL_KEY = "efl:local:v3";
const online = Boolean(DB_URL && DB_TOKEN);
...
/** Brings older saves (v1/v2) up to date without losing anything. */
function migrate(s) {
  if (!s) return defaultState();
  const next = { ...defaultState(), ...s };
  next.players = (s.players || DEFAULT_PLAYERS).map((p) => ({ ...p }));
  // v1 baked friendly rating changes into the stored rating — undo that,
  // because we now replay ratings from scratch every time.
  if ((s.version || 1) < 2) {
...
function computeTable(players, matches) {
  const rows = new Map(
    players.map((p) => [
      p.name,
      { name: p.name, seed: p.rating, p: 0, w: 0, d: 0, l: 0, gf: 0, ga: 0, gd: 0, pts: 0, seq: [] },
    ])
  );
  ...
  list.sort((a, b) =>
    b.pts - a.pts || b.gd - a.gd || b.gf - a.gf || h2h(a, b) || b.seed - a.seed || a.name.localeCompare(b.name)
  );
...
function strengthsFrom(ratings) {
...
function lambdas(strengths, home, away, homeAdv) {
...
/**
 * Live ratings are replayed from the seed every time, in date order.
 * League matches move them fully, friendlies at 35%. Because nothing
 * is stored, fixing an old score fixes every rating automatically.
 */
function computeLiveRatings(players, leagueMatches, friendlies, homeAdvantage) {
  ...
  for (const m of events) {
    ...
    const st = strengthsFrom(ratings);
    const [lh, la] = lambdas(st, m.home, m.away, m.kind === "league" && homeAdvantage);
    const p = outcome(lh, la);
    const expected = p.home + 0.5 * p.draw;
  ...
  for (const p of players) {
    ...
    out.set(p.name, { seed: p.rating, live, delta: live - p.rating, leagueShift: s.league, friendlyShift: s.friendly });
  }
  return out;
}
...
function computeStrengths(players, liveRatings, table) {
  ...
  const base = strengthsFrom(ratings);
...
const predict = (home, away, strengths, homeAdv) => outcome(...lambdas(strengths, home, away, homeAdv));
...
function simulateSeason(players, matches, strengths, homeAdvantage) {
  const remaining = matches.filter((m) => !m.played);
  if (!remaining.length) return null;
  const base = new Map(computeTable(players, matches).map((r) => [r.name, { pts: r.pts, gd: r.gd, gf: r.gf }]));
  const lam = remaining.map((m) => ({ home: m.home, away: m.away, l: lambdas(strengths, m.home, m.away, homeAdvantage) }));
  ...
  for (let s = 0; s < SIMS; s++) {
    ...
    const ranked = [...acc.values()].sort((x, y) => y.pts - x.pts || y.gd - x.gd || y.gf - x.gf || Math.random() - 0.5);
...
/** Coin balances are replayed from the bet list, same idea as ratings. */
function computeBalances(players, bets, results) {
  const out = new Map();
  const ensure = (n) => {
    if (!out.has(n)) out.set(n, { balance: START_COINS, held: 0, open: 0, won: 0, lost: 0 });
    return out.get(n);
  };
  players.forEach((p) => ensure(p.name));
  for (const bet of bets || []) {
    const acc = ensure(bet.who);
    const r = (results || {})[bet.matchId];
    if (!r) { acc.balance -= bet.stake; acc.held += bet.stake; acc.open++; continue; }
    const res = r.hg > r.ag ? "home" : r.hg < r.ag ? "away" : "draw";
    if (res === bet.pick) { acc.balance += Math.round(bet.stake * (bet.odds - 1)); acc.won++; }
...
async function loadState() {
  ...
  const res = await fetch(`${DB_URL}/get/${STORE_KEY}`, {
    headers: { Authorization: `Bearer ${DB_TOKEN}` },
  });
  if (!res.ok) throw new Error("Couldn't reach the league database.");
  const data = await res.json();
  return data.result ? migrate(JSON.parse(data.result)) : defaultState();
}

async function saveState(state) {
  if (!online) return writeLocal(state);
  const res = await fetch(`${DB_URL}/set/${STORE_KEY}`, {
    method: "POST",
    headers: { Authorization: `Bearer ${DB_TOKEN}`, "Content-Type": "text/plain" },
    body: JSON.stringify(state),
  });
  if (!res.ok) throw new Error("Couldn't save. Check your connection.");
  return state;
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (alive) { setState(s); setSync("ok"); }
      } catch (e) {
        if (alive) { setSync("error"); setError(e.message); }
      }
      try {
        const saved = localStorage.getItem(ME_KEY);
        if (alive && saved) setMe(saved);
  ...
  /** Every change goes through here: reload, apply, save, log. */
  const commit = useCallback(async (mutate, logText) => {
    setSync("saving");
    setError(null);
    try {
      ...
      if (logText) {
        next.log = [
          { id: `${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, who: me || "Someone", text: logText, ts: Date.now() },
          ...(next.log || []),
        ].slice(0, 120);
      }
  ...
  const chooseMe = useCallback((name) => {
    setMe(name);
    setPicking(false);
    try { localStorage.setItem(ME_KEY, name); } catch { /* storage blocked */ }
  }, []);
  ...
  const balances = useMemo(
    () => computeBalances(state.players, state.bets || [], state.results || {}),
    [state.players, state.bets, state.results]
  );
      ...
      <div className="wrap">
        <header className="head">
          ...
          <div className="head-text">
            <p className="eyebrow">{state.season} · {state.players.length} players · {played}/{matches.length} played</p>
            <h1>EFootball Elite League</h1>
          </div>
          <div className="head-side">
            <span className={`pill sync-${sync}`}>
              {{ loading: "Loading", ok: online ? "Shared" : "This browser", saving: "Saving…", error: "Offline" }[sync]}
            </span>
            {online && <button className="ghost" onClick={pull}>Refresh</button>}
            {me ? (
              <button className="ghost who" onClick={() => setPicking(true)}>
                {me}<span className="mono coinchip">{balances.get(me)?.balance ?? START_COINS}c</span>
              </button>
            ) : (
              <button className="primary" onClick={() => setPicking(true)}>Who are you?</button>
            )}
          </div>
        </header>
        ...
        {!online && (
          <p className="banner">
            Saving in this browser only. To share one league with everyone, paste your Upstash
            URL and token into the two lines at the top of App.jsx.
          </p>
        )}

        {(picking || (!me && ready)) && (
          <NamePicker players={state.players} me={me} onPick={chooseMe} onClose={me ? () => setPicking(false) : null} />
        )}
        ...
        {!ready ? (
          ...
          <>
            {tab === "league" && (
              <League state={state} table={table} matches={matches} predictor={predictor} me={me} commit={commit} editing={editing} />
            )}
            {tab === "friendly" && (
              <Friendly state={state} me={me} commit={commit} editing={editing} />
            )}
            {tab === "predict" && (
              <Predictions
                state={state} matches={matches} strengths={strengths} predictor={predictor}
                me={me} balances={balances} commit={commit} onPick={() => setPicking(true)}
              />
            )}
            {tab === "coins" && (
              <Coins state={state} matches={matches} me={me} balances={balances} commit={commit} onPick={() => setPicking(true)} />
            )}
            {tab === "players" && (
              <Players state={state} table={table} matches={matches} liveRatings={liveRatings} me={me} commit={commit} />
            )}
            {tab === "stats" && <Stats table={table} matches={matches} liveRatings={liveRatings} />}
            {tab === "log" && <Activity state={state} me={me} commit={commit} />}
          </>
        )}
        ...
        <footer className="foot">
          <p>
            {online
              ? "Everyone edits the same league. Changes show up for others within 15 seconds."
              : "Running in this browser only."} Coins are for fun, never real money.
          </p>
        </footer>
      </div>
    </div>
...
function NamePicker({ players, me, onPick, onClose }) {
  return (
    <section className="card ident">
      <div className="card-head bar">
        <div>
          ...
          <p className="muted sm">
            Just pick your name — no password. It's saved on this device, and it signs your edits
            in the activity log so everyone can see who changed what.
          </p>
        </div>
      ...
      <div className="chips">
        {players.map((p) => (
          <button key={p.name} className={`chip${me === p.name ? " sel" : ""}`} onClick={() => onPick(p.name)}>
            {p.name}
          </button>
        ))}
        <button className={`chip quiet${me === "Guest" ? " sel" : ""}`} onClick={() => onPick("Guest")}>
          Just watching
        </button>
      </div>
    </section>
...
function League({ state, table, matches, predictor, me, commit, editing }) {
  ...
  return (
    <section>
      ...
      {sub === "table" ? (
        <TableView table={table} total={matches.length} />
      ) : (
        <Fixtures state={state} matches={matches} predictor={predictor} me={me} commit={commit} editing={editing} />
      )}
    </section>
  );
}
  ...
  return (
    <div className="card">
      <div className="card-head">
        ...
        <p className="muted sm">League matches only — friendlies never touch this. Points, then GD, then goals for, then head-to-head.</p>
      </div>
          ...
          <tbody>
            {table.map((r, i) => (
              <tr key={r.name} className={`row-in${r.pos === 1 ? " lead" : ""}`} style={{ animationDelay: `${i * 40}ms` }}>
                ...
                <td className="name">{r.name}{done && r.pos === 1 && <span className="crown">Champion</span>}</td>
      ...
      {table.every((r) => r.p === 0) && <p className="empty">No results yet. Open Fixtures and add the first score.</p>}
    </div>
  );
}
...
function Fixtures({ state, matches, predictor, me, commit, editing }) {
    ...
    <div>
      <div className="card-head bar">
        <div>
          ...
          <p className="muted sm">Everyone can enter any result — tap a match to open it.</p>
        </div>
        <div className="seg">
        ...
        <label className="field">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (v) commit((s) => { s.startDate = v; }, `Moved the season opener to ${fmtDate(v)}`);
            }}
          />
        </label>
        <p className="muted sm">Shifts every matchday that hasn't been given its own date.</p>
      </div>
        ...
        return (
          <div className="card" key={round}>
            <div className="round-head">
              ...
              <span className={`pill ${done === ms.length ? "done" : "todo"}`}>{done}/{ms.length} played</span>
            </div>
            <ul className="matches">
              {ms.map((m) => (
                <li key={m.id} className={`match${open === m.id ? " open" : ""}`}>
                  <button className="match-btn" onClick={() => setOpen(open === m.id ? null : m.id)} aria-expanded={open === m.id}>
                    ...
                    <span className="match-meta">
                      {m.played ? <span className="pill done">FT</span> : <ProbBar pred={predictor(m.home, m.away)} compact />}
                    </span>
                  </button>
                  {open === m.id && (
                    <MatchEditor m={m} pred={predictor(m.home, m.away)} me={me} commit={commit} editing={editing} />
                  )}
                </li>
              ))}
            </ul>
...
function MatchEditor({ m, pred, me, commit, editing }) {
  ...
  return (
    <div className="editor">
      <div className="pred-full">
        <ProbBar pred={pred} />
        <p className="mono sm muted">Expected goals {pred.lh.toFixed(2)} – {pred.la.toFixed(2)} · most likely {pred.likely}</p>
      </div>
        ...
        <label className="field">
          ...
          <input
            ...
            onChange={(e) => {
              const v = e.target.value;
              setDate(v);
              if (v && v !== m.date) commit((s) => { s.dates[m.id] = v; }, `Moved ${m.home} v ${m.away} to ${fmtDate(v)}`);
            }}
          />
        </label>
        <div className="btns">
          <button
            className="primary" disabled={!valid}
            onClick={() => {
          ...
          {m.played && (
            <button
              ...
              onClick={() => commit((s) => { delete s.results[m.id]; }, `Cleared ${m.home} v ${m.away} · MD${m.round}`)}
            >
              Clear result
            </button>
          )}
      ...
      <p className="muted sm">Saving as <b>{me || "Someone"}</b> — change that with the button up top.</p>
    </div>
  );
}
...
function Friendly({ state, me, commit, editing }) {
  const names = useMemo(() => state.players.map((p) => p.name), [state.players]);
  const list = useMemo(
  ...
  useEffect(() => {
    editing.current = editId !== null || hg !== "" || ag !== "";
    return () => { editing.current = false; };
  }, [editing, editId, hg, ag]);

  const selected = list.find((m) => m.id === editId) || null;
  useEffect(() => {
    if (!selected) {
      setHome(me && names.includes(me) ? me : names[0] || "");
      setAway(names.find((n) => n !== me) || names[1] || "");
      setDate(todayISO());
      setHg(""); setAg("");
      return;
    }
  ...
  }, [editId, selected, names, me]);

  const ok = home && away && home !== away && date &&
    ((hg === "" && ag === "") || (hg !== "" && ag !== ""));
  ...
  const save = () => {
    ...
    const now = Date.now();
    const id = editId || `f-${now}-${Math.random().toString(36).slice(2, 6)}`;
    commit((s) => {
      ...
      if (idx >= 0) s.friendlies[idx] = match; else s.friendlies.unshift(match);
    }, scored ? `Friendly: ${home} ${h}–${a} ${away}` : `Friendly scheduled: ${home} vs ${away} on ${fmtDate(date)}`);
    setEditId(null);
  };
    ...
    <section>
      <div className="card-head bar">
        <div>
          ...
          <p className="muted sm">
            Extra games outside the league. No effect on the table or points — but a played friendly
            moves both ratings by {Math.round(FRIENDLY_WEIGHT * 100)}% of a league match, so predictions still learn from it.
          </p>
        </div>
      </div>
      ...
      <div className="card">
        <div className="friendly-grid">
          ...
          <div className="friendly-pair">
            ...
            <div className="score-edit start">
          ...
          <div className="btns">
            <button className="primary" onClick={save} disabled={!ok}>
              {editId ? "Update friendly" : hg !== "" || ag !== "" ? "Save friendly" : "Schedule friendly"}
            </button>
            {editId && <button className="ghost" onClick={() => setEditId(null)}>Cancel edit</button>}
          </div>
        </div>
      </div>
              ...
              <li key={m.id} className="match">
                <div className="match-btn asrow">
                  ...
                  <span className="match-meta friendlymeta">
                    ...
                    <button className="ghost small" onClick={() => setEditId(m.id)}>Edit</button>
                    <button
                      ...
                      onClick={() => {
                        ...
                        }, `Removed friendly ${m.home} vs ${m.away}`);
...
function Predictions({ state, matches, strengths, predictor, me, balances, commit, onPick }) {
  ...
  const myCoins = me && me !== "Guest" ? balances.get(me)?.balance ?? START_COINS : 0;
  const myBets = (state.bets || []).filter((b) => b.who === me && !(state.results || {})[b.matchId]);
    ...
    <section>
      <div className="card-head bar">
        <div>
          ...
          <p className="muted sm">Odds come from live ratings and recent form. Favourite = small safe win, underdog = big payout.</p>
        </div>
        <label className="toggle">
          <input
            ...
            onChange={(e) => {
              ...
              commit((s) => { s.homeAdvantage = on; }, `Turned home advantage ${on ? "on" : "off"}`);
            }}
          />
      ...
      {(!me || me === "Guest") && (
        <div className="card sub bar">
          <p className="muted sm">Pick your name to bet coins — everyone starts with {START_COINS}.</p>
          <button className="ghost" onClick={onPick}>Pick your name</button>
        </div>
      )}
      ...
      <div className="card">
        <div className="round-head">
          ...
          <div className="seg tight">
            ...
            <button className="segb" onClick={() => setRound(Math.min(rounds.at(-1), round + 1))} disabled={round >= rounds.at(-1)}>Next</button>
          </div>
        </div>
        <div className="pcards">
          {slate.map((m) => (
            <PredictionCard
              key={m.id} m={m} pred={predictor(m.home, m.away)} me={me} myCoins={myCoins}
              myBet={myBets.find((b) => b.matchId === m.id)} commit={commit}
            />
          ))}
        </div>
      </div>
      ...
      {title && (
        <div className="card">
          <div className="card-head">
            ...
            <p className="muted sm">{SIMS.toLocaleString()} simulated seasons from today's table.</p>
          </div>
          <ul className="odds">
            {title.map((o) => (
              <li key={o.name}>
...
function PredictionCard({ m, pred, me, myCoins, myBet, commit }) {
  const [pick, setPick] = useState(null);
  const [stake, setStake] = useState("");
  const [busy, setBusy] = useState(false);
  const odds = oddsForMatch(pred);
  const playing = me && (m.home === me || m.away === me);
  ...
  return (
    <article className="pcard">
      ...
      <ProbBar pred={pred} />
      <p className="mono sm muted center">xG {pred.lh.toFixed(2)} – {pred.la.toFixed(2)} · likely {pred.likely}</p>
          ...
          {myBet ? (
            <div className="mybet">
              <span className="sm">
                ...
                <span className="muted"> → {Math.round(myBet.stake * myBet.odds)}c</span>
              </span>
              <button
                className="ghost small danger"
                onClick={() => commit((s) => {
                  const i = (s.bets || []).findIndex((b) => b.id === myBet.id);
                  if (i >= 0) s.bets.splice(i, 1);
                }, `Cancelled a ${myBet.stake}-coin bet`)}
              >
                Cancel
              </button>
            </div>
            ...
            <p className="muted sm">Pick your name to bet on this match.</p>
          ) : (
            <>
              <div className="oddsrow">
                {["home", "draw", "away"].map((p) => (
                  <button key={p} className={`oddsbtn ob-${p}${pick === p ? " on" : ""}`} onClick={() => setPick(pick === p ? null : p)}>
              ...
              {pick && (
                <>
                  <div className="stakerow">
                    <input
                      className="input mono" inputMode="numeric" placeholder={`Stake (min ${MIN_STAKE})`}
                      value={stake} onChange={(e) => setStake(e.target.value.replace(/[^0-9]/g, "").slice(0, 5))}
                    />
                    <button
                      ...
                      onClick={async () => {
                        ...
                        const done = await commit((s) => {
                          ...
                          s.bets.push({
                            ...
                            who: me, matchId: m.id, pick, stake: n, odds: odds[pick], ts: Date.now(),
                          });
                        }, `Bet ${n} coins on ${label(pick)} at ${odds[pick].toFixed(2)} (${m.home} v ${m.away})`);
                        setBusy(false);
                        if (done) { setPick(null); setStake(""); }
                      }}
                    >
                  ...
                  {stake !== "" && !stakeOk && (
                    <p className="muted sm">{n < MIN_STAKE ? `Minimum stake is ${MIN_STAKE} coins.` : `You only have ${myCoins} coins.`}</p>
                  )}
                </>
              )}
            </>
...
function Coins({ state, matches, me, balances, commit, onPick }) {
  const results = state.results || {};
  const bets = state.bets || [];
  const byId = useMemo(() => new Map(matches.map((m) => [m.id, m])), [matches]);

  const board = [...balances.entries()]
    .map(([name, b]) => ({ name, ...b }))
    .sort((a, b) => b.balance - a.balance);

  const mine = me ? bets.filter((b) => b.who === me).sort((a, b) => b.ts - a.ts) : [];
  const open = bets.filter((b) => !results[b.matchId]).sort((a, b) => b.ts - a.ts);

  const describe = (b) => {
    const m = byId.get(b.matchId);
    const what = !m ? b.matchId : b.pick === "draw" ? `${m.home} v ${m.away} draw` : b.pick === "home" ? m.home : m.away;
  ...
  return (
    <section>
      <div className="card-head">
        ...
        <p className="muted sm">
          Everyone starts with {START_COINS}. Bet on match predictions — favourites pay little,
          underdogs pay big. Just for fun, never real money, and no effect on the table or ratings.
        </p>
      </div>
      ...
      {me && me !== "Guest" ? (
        <div className="card coincard">
          <div className="coinbig">
            <span className="mono coinnum">{balances.get(me)?.balance ?? START_COINS}</span>
            <span className="rating-lbl">your coins</span>
          </div>
          <dl className="stats-row coinstats">
            <Stat k="In play" v={balances.get(me)?.held ?? 0} />
            <Stat k="Open" v={balances.get(me)?.open ?? 0} />
            <Stat k="Won" v={balances.get(me)?.won ?? 0} tone="win" />
            <Stat k="Lost" v={balances.get(me)?.lost ?? 0} tone="loss" />
          </dl>
        </div>
      ) : (
        <div className="card sub bar">
          <p className="muted sm">Pick your name to get your {START_COINS} coins.</p>
          <button className="ghost" onClick={onPick}>Pick your name</button>
        </div>
      )}
      ...
      <div className="two">
          ...
          <ul className="formlist">
            {board.map((r, i) => (
              <li key={r.name}>
                ...
                <span className="mono sm muted">{r.won}W {r.lost}L</span>
                <span className="mono pts">{r.balance}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="card">
          <h3 className="h3">{me && me !== "Guest" ? "Your bets" : "Open bets"}</h3>
          {me && me !== "Guest" ? (
            !mine.length ? (
              ...
              <ul className="betlist">
                {mine.slice(0, 12).map((b) => {
                  ...
                  return (
                    <li key={b.id}>
                      <span className="betwhat"><b>{b.stake}c</b> on {what} <span className="mono muted">@ {b.odds.toFixed(2)}</span></span>
                      <span className={`pill ${status.cls}`}>{status.text}</span>
                      {!results[b.matchId] && (
                        <button
                          className="ghost small danger"
                          onClick={() => commit((s) => {
                            const i = (s.bets || []).findIndex((x) => x.id === b.id);
                            if (i >= 0) s.bets.splice(i, 1);
                          }, `Cancelled a ${b.stake}-coin bet`)}
                        >
                          Cancel
                        </button>
                      )}
            ...
            <ul className="betlist">
              {open.slice(0, 12).map((b) => {
                ...
                return (
                  <li key={b.id}>
                    <span className="betwhat"><b>{b.who}</b>: {b.stake}c on {what} <span className="mono muted">@ {b.odds.toFixed(2)}</span></span>
                  </li>
                );
              })}
            </ul>
...
function Players({ state, table, matches, liveRatings, me, commit }) {
    ...
    <section>
      <div className="card-head bar">
        <div>
          ...
          <p className="muted sm">
            Live rating moves with every result — league at full weight, friendlies at {Math.round(FRIENDLY_WEIGHT * 100)}%.
            The seed is just where the season started.
          </p>
        </div>
            ...
            <article className={`card player${isOpen ? " open" : ""}`} key={p.name}>
              <header className="player-head">
                  ...
                  {setup ? (
                    <input
                      ...
                      onBlur={(e) => {
                        ...
                        if (v !== p.rating) {
                          commit((s) => {
                            const t = s.players.find((x) => x.name === p.name);
                            if (t) t.rating = v;
                          }, `Set ${p.name}'s seed rating to ${v}`);
                        }
                      }}
                ...
                <div className="player-id">
                  <h3 className="h3">{p.name}{me === p.name && <span className="youchip">you</span>}</h3>
                  <p className="mono sm muted">seed {p.rating} · {r.p ? `${ordinal(r.pos)} · ${r.pts} pts` : "yet to play"}</p>
                </div>
              </header>

              <dl className="stats-row">
                <Stat k="P" v={r.p} /><Stat k="W" v={r.w} tone="win" />
                <Stat k="D" v={r.d} /><Stat k="L" v={r.l} tone="loss" />
                <Stat k="GF" v={r.gf} /><Stat k="GA" v={r.ga} />
  ...
  return (
    <div className="h2h">
      <table className="grid tight">
        <thead>
          <tr><th>Opponent</th><th className="num">P</th><th className="num">W</th><th className="num">D</th><th className="num">L</th><th className="num">Goals</th></tr>
        </thead>
        <tbody>
          {rows.map((r) => (
            <tr key={r.opp}>
...
function Stats({ table, matches, liveRatings }) {
  ...
  const run = active.map((r) => ({ name: r.name, n: longestRun(r.seq, "W") })).sort((a, b) => b.n - a.n)[0];
    ...
    <section>
      <div className="awards">
        ...
        <Award
          ...
          name={run.n ? run.name : "Nobody yet"}
          value={run.n ? `${run.n} straight win${run.n > 1 ? "s" : ""}` : "No back-to-back wins so far"}
          tone="win"
        />
        <Award
          title="Biggest improvement"
          name={riser && riser.delta >= 0.5 ? riser.name : "Nobody yet"}
          value={riser && riser.delta >= 0.5
            ? `Rating up ${riser.delta.toFixed(1)} (${riser.seed} → ${Math.round(riser.live)})`
            : "No one has outgrown their seed yet"}
          tone="draw"
        />
        <Award
...
function Activity({ state, me, commit }) {
  const log = state.log || [];
  ...
  return (
    <section className="card">
      <div className="card-head">
        ...
        <p className="muted sm">Every change, newest first, signed with whoever's name was picked. Keeps the last 120.</p>
      </div>
      {!log.length ? (
      ...
      <div className="reset">
        {!confirming ? (
          ...
          <div className="btns">
            <span className="muted sm">Clears every result, friendly, bet and the log. Seed ratings stay.</span>
            <button
              className="primary danger"
              onClick={() => {
                commit((s) => { s.results = {}; s.friendlies = []; s.bets = []; s.log = []; },
                  `${me || "Someone"} started a new season`);
                setConfirming(false);
              }}
            >
              Clear the season
...
.efl .who { display: inline-flex; align-items: center; gap: 8px; font-weight: 700; }
.efl .coinchip { color: var(--home); font-size: 11px; }
.efl .youchip { font-family: var(--mono); font-size: 9px; letter-spacing: 0.1em; text-transform: uppercase; color: var(--home); border: 1px solid rgba(242,166,59,0.4); padding: 1px 5px; margin-left: 8px; vertical-align: middle; }
...
.efl .card-head.bar, .efl .card.sub.bar { display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; }
...
.efl .ident .chips { display: flex; flex-wrap: wrap; gap: 8px; }
.efl .chip { background: var(--panel2); border: 1px solid var(--line); color: var(--paper); padding: 9px 16px; font-weight: 700; letter-spacing: -0.01em; font-size: 14px; }
.efl .chip:hover { border-color: var(--home); color: var(--home); }
.efl .chip.sel { border-color: var(--home); color: var(--home); background: rgba(242,166,59,0.1); }
.efl .chip.quiet { color: var(--muted); font-weight: 400; }

.efl .seg { display: flex; border: 1px solid var(--line); }
.efl .seg.inline { display: inline-flex; margin-bottom: 14px; }
...
.efl .score-edit.start { justify-content: flex-start; }
